...
describe('UncontrolledForm Component', () => {
  ...
  test('Should show error if name starts with lowercase letter after submit', async () => {
    renderWithStore();

    const nameInput = screen.getByLabelText(/name/i);
    await userEvent.type(nameInput, 'j6hn');

    expect(screen.queryByText('First letter must be uppercase')).not.toBeInTheDocument();

    await userEvent.click(screen.getByRole('button', { name: /ready to rock/i }));

    expect(await screen.findByText('First letter must be uppercase')).toBeInTheDocument();
  });

  test('Should show error if passwords do not match after submit', async () => {
    renderWithStore();
  ...
  test('Should reject country if it is not in the Redux store list after submit', async () => {
